...
# 파일 생성
# 파일 생성시 open(경로, 쓰기/읽기모드, 인코딩, 뉴라인) 이런 식으로 적어야 한다. 뉴라인은 윈도우에서 이상한 줄바꿈문자가 자동으로 들어가는데 줄바꿈문자를 없에기 위해 적는다.
f = open(r"C:\Users\USER\OneDrive\바탕 화면\startcoding\02_Startcoding_crawling\03_네이버_쇼핑_크롤링\data.csv", 'w', encoding='CP949', newline='')
csvWriter = csv.writer(f)   # csv파일 읽기.
...
for item in items:
    ...
    # CSV 데이터 쓰기
    csvWriter.writerow([name, price, link])      # csv파일 쓰기를 실행하는데 row(행) 형태로 namd, price, link를 저장하겠다는 뜻이다.
