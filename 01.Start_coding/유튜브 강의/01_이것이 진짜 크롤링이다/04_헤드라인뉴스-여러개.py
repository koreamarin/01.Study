...
# select_one과 다르게 여러개의 정보를 담고 있어서 리스트형태로 각 정보가 저장된다.
# class의 속성으로 select할 때는 .으로 대체하여 쓰도록 한다.
titles = soup.select('.lik_hdline_article')

# 요청을 보낸 URL의 class : lik_hdline_article를 찾아 text부분만 출력
# strip()은 양쪽 공백을 제거한다는 뜻.
# print(titles.text.strip())을 사용하면 리스트형태로 출력되기 때문에 for문을 사용하여 title만 추출한다.
for title in titles :
